...
@Component({
  ...
  styleUrls: ['./ng-select.component.scss', './ng-select.themes.scss'],
...
export class NgSelectComponent extends BaseComponent implements OnDestroy, OnChanges, AfterViewInit, ControlValueAccessor, OnInit {
  ...
  @Input() labelForId: string | number = newId()
  @Input() label: string
  @Input() isDisplayField = false
  @Input() expandableErrors = false
  @Input() inputAttrs: { [key: string]: string } = {}
  @Input() tabIndex: number
  @Input() readonly = false
  @Input() searchWhileComposing = true
  @Input() minTermLength = 3
  @Input() emitTypeaheadOnClearTerm = false

  @Input() addOption: any
  @Input() requestObject: RequestObject

  @Input() itemsObserver: (r: RequestObject) => Observable<any>
  @Input() forceUpdateList = false
  ...
  @Output() emitSelectedObject = new EventEmitter<any>()
  // custom templates
  ...
  private get _validTerm() {
    ...
    return (term && term.length >= this.minTermLength) || (!term && this.emitTypeaheadOnClearTerm)
  }
  ...
  ngOnInit() {
    ...
    if (!this.labelForId && this.ngControl.name) {
      this.labelForId = this.ngControl.name
    }

    this.ngControl.statusChanges!.subscribe(() => this._cd.markForCheck())

    if (this.itemsObserver) {
      this.typeahead = new Subject<string>()
      // this.emitTypeaheadOnClearTerm = true
      ...
      this.typeahead.pipe(
        ...
        filter(value => !!value.toString().trim() || value.toString().trim().length >= this.minTermLength),
        switchMap(
          (term: string) => {
            if (term) {
              this.request.search['searchString'] = term.trim()
            } else {
              delete this.request.search['searchString']
            }
            return this.itemsObserver(this.request).pipe(
              catchError(() => of({list: [], totalCount: 0})),
              tap(() => {
                this.loading = true
                this.resetItems()
      ...
      ).subscribe(this.handleItemsResponse)
    }
  }

  ngOnChanges(changes: SimpleChanges) {
    if (changes.requestObject) {
      this.request = this.requestObject
      this.items
    }
    if (changes.multiple) {
    ...
    if (changes.items) {
      ...
      if (this.itemsObserver && changes.items.currentValue) {
        this._setItems(
          this.addOption ?
            [...changes.items.currentValue, this.addOption, ...this.allItems] :
            [...changes.items.currentValue, ...this.allItems]
        )
      }
    }
    if (changes.isOpen) {
  ...
  ngAfterViewInit() {
    if (!this._itemsAreUsed && !this.itemsObserver) {
  ...
  loadList() {
    this.loading = true
    this.subs.add(
      this.itemsObserver(this.request).subscribe(this.handleItemsResponse)
    )
  }
  ...
  handleKeyCode($event: KeyboardEvent) {
    switch ($event.which) {
      ...
      case KeyCode.Tab:
        ...
        break
      case KeyCode.Esc:
        this.isOpen && $event.stopPropagation()
        this.close()
  ...
  open() {
    ...
    // if (this.itemsObserver && (!this.allItems.length || this.forceUpdateList)) {
  ...
  select(item: NgOption) {
    if (!item.selected) {
      ...
      this.emitSelectedObject.emit(item.value)
...
// @ts-ignore
  ...
  filter(term: string) {
    ...
    if (!this._isTypeahead && !this.itemsObserver) {
...
// @ts-ignore
  ...
  private handleItemsResponse = (response: any) => {
    this.loading = false
    // this.total = response.totalCount
    this.allItems = [...this.allItems, ...response]
    this._setItems(this.addOption ? [this.addOption, ...this.allItems] : [...this.allItems])
    this.detectChanges()
  }
  ...
  private _setItemsFromNgOptions() {
    ...
    const handleOptionChange = () => {
      ...
      merge(...this.ngOptions.map((option: NgOptionComponent) => option.stateChange$))
  ...
  private _setInputAttributes() {
    ...
    const attributes = {
      ...
      autocorrect: 'off',
      autocapitalize: 'off',
      autocomplete: this.labelForId ? 'nope' : this.dropdownId,
      ...this.inputAttrs
    }
